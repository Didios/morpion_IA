...
class morpion:
    ...
    def entrainement(self):
        ...
        repetitions = Scale(self.root, orient='horizontal', from_=50, to=10000, resolution=1, length=500) #50-10 000
        repetitions.pack()
...
il existe des choses insensée dans leur cerveau genre 022000000 ou bien 000200102

il y a aussi des problèmes d'accès aux fichiers, ce qui arrive de manière totalement aléatoire (peut-être car main_interface.py est ouvert ?
"""
"""
En soi, l'IA n"as pas besoin de faire la différence entre O et X, donc on peut représenter les deux de la même manière:
    on obtiendrat juste des suites de 0 et de 1
"""
class IA:
    """
    ...
    def debut_partie(self):
        ...
        parametres:
            premier, un booléen qui indique si c'est l'IA qui joue en premier
        """
        ...
        def creation_arbre(dico, base):
            ...
            dico_arbre = {}
            for keys in dico.keys():
                if dico[keys][0] not in dico.keys():
                    dico_arbre[keys] = arbre(keys, arbre(int(dico[keys][0])))
                else:
                    dico_arbre[keys] = arbre(keys)

            for racine, abr in dico_arbre.items():
                for fils in dico[racine]:
                    if fils in dico_arbre.keys():
                        abr.add_fils(dico_arbre[fils])

            return dico_arbre[base]
    ...
    def jouer(self, plateau):
        ...
        if choix[1] < 1 and len(self.reflexion.get_fils()) < self.nbr_possibilite(situation):
            ...
            situation = self.plateau_conversion_chaine(plateau)
            pos = self.nbr_possibilite(situation)
            i = 0
            while i < len(situation):
                if situation[i] == "0":
                    s = situation
                    """
                    je pense que la pbm signalé au début vient de là, genre il met le mauvais ou le plateau est perturbant
                    peut_être changé le plateau-->chaine quand on est first
                    """
                    if pos % 2 == 0:
                        s = s[:i] + "2" + s[i+1:]
                    else:
